...
parser = argparse.ArgumentParser(description='Predict the class of each pixel for an image and save the result.')
parser.add_argument('-model',type=str,default='model_inria.pt',help='A saved pytorch model')
parser.add_argument('-inpfile',type=str,default='/exports/csce/eddie/geos/groups/geos_cnn_imgclass/data/AerialImageDataset/test/images/sfo7.tif',help='Path and filename of image to be classified')
...
def image_plotter(net,image,RGB_image,fname):
    output = net(image)
    ...
    fig=plt.figure(figsize=(10, 15), dpi= 300, facecolor='w', edgecolor='k')
    ax1 = plt.subplot(1,2,1)
    ...
    patches = [(mpatches.Patch(color='white', label="Building")),(mpatches.Patch(color='black', label="No Buildings"))]
    ax3.legend(handles=patches, bbox_to_anchor=(1.05, 1), loc="upper right", borderaxespad=0.,facecolor="plum" )
    ax1.imshow(RGB_image)
    plt.savefig('Predicted_{}.png'.format(fname),bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    image_plotter(net,image,RGB_image,fname)
